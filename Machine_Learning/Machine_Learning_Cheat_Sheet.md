...
| **Lasso Regression**    | Adds L1 penalty to regression for feature selection.  | Prevent overfitting, shrink weights    | Sparse datasets, high-dimensional data    | Loss = RSS + λ(slope)²                                                         | `sklearn.linear_model.Lasso`                                  |
| **Ridge Regression**    | Adds L2 penalty to regression to shrink coefficients. | Regularized regression                 | Multicollinearity problems                | oss = RSS + λ mod(slope)                                                          | `sklearn.linear_model.Ridge`                                  |
| **Elastic Net**         | Combines L1 + L2 regularization.                      | Balanced feature selection & shrinkage | Correlated predictors                     | Loss = Lasso + Ridge                                            | `sklearn.linear_model.ElasticNet`                             |
...
| **SVM**                 | Finds optimal hyperplane between classes.             | Maximize margin                        | Image classification, OCR                 | f(x) = w·x + b; max margin = 2 / //w//                                      | `sklearn.svm.SVC`, `SVR`                                |
...
| **Naive Bayes**         | Probabilistic classifier using Bayes' Theorem.        | Fast and simple classification         | Email spam, sentiment analysis            | `P(AuB) = P(BuA)*P(A)/P(B)`                                                   | `sklearn.naive_bayes.GaussianNB`, `MultinomialNB`            |
